...
Public Class ReportControl
    ...
    Private Sub ReportControl_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        LoadTransactionData(reportData, tbSearchTransaction.Text.Trim(), fromDate, toDate)

        ' Display additional information (e.g., sales)
        DisplayTodaySales()
        DisplayMonthlySales()
    End Sub
    ...
    Sub LoadTransactionData(existingDataGridView As DataGridView, filterText As String, Optional ByVal fromDate As DateTime = Nothing, Optional ByVal toDate As DateTime = Nothing)
        Try
            Using connection As New MySqlConnection(connectionString)
                ...
                ' Load all transaction data without any filtering
                Dim query As String = "SELECT o.transaction_id, o.product_name, o.sub_total, o.quantity, o.grand_total, o.transaction_date, CONCAT(e.employee_fname, ' ', e.employee_lname) AS cashier FROM orders o INNER JOIN employee e ON o.employee_id = e.employee_id"
                ...
                If fromDate <> DateTime.MinValue AndAlso toDate <> DateTime.MinValue Then
                    query &= $" WHERE o.transaction_date BETWEEN '{fromDate.ToString("yyyy-MM-dd")}' AND '{toDate.ToString("yyyy-MM-dd")}'"
                End If
                ...
                If Not String.IsNullOrWhiteSpace(filterText) Then
                    ...
                    query &= $"CONCAT(o.transaction_id, o.product_name, ' ', o.employee_id, ' ', CONCAT(e.employee_fname, ' ', e.employee_lname)) LIKE '%{filterText}%'"
                End If
                ...
                Using command As New MySqlCommand(query, connection)
                    Using adapter As New MySqlDataAdapter(command)
                        ...
                        existingDataGridView.DataSource = dataSet.Tables("orders")
                    End Using
                End Using
            End Using
    ...
    Private Sub tbSearchTransaction_TextChanged(sender As Object, e As EventArgs) Handles tbSearchTransaction.TextChanged
        ' Handle the filter logic when the search text changes
        ' Load all transactions without applying the date filter
        LoadTransactionData(reportData, tbSearchTransaction.Text.Trim())
    End Sub
    ...
    Sub DisplayTodaySales()
        Try
            ...
            Using connection As New MySqlConnection(connectionString)
                ...
                Dim query As String = $"SELECT SUM(grand_total) FROM orders WHERE transaction_date = '{todayDate.ToString("yyyy-MM-dd")}'"

                Using command As New MySqlCommand(query, connection)
                    Dim todaySales As Object = command.ExecuteScalar()

                    If todaySales IsNot DBNull.Value Then
                        ' Format sales amount with currency symbol, commas, and "M" for millions
                        lbl_todaySales.Text = $"₱ {Convert.ToDecimal(todaySales).ToString("N2", CultureInfo.InvariantCulture)}"
                    Else
                        lbl_todaySales.Text = "₱ 0.00"
                    End If
                End Using
            End Using
        Catch ex As Exception
            MessageBox.Show($"Error calculating today's sales: {ex.Message}")
        End Try
    End Sub

    Sub DisplayMonthlySales()
        Try
            ' Get the start and end dates of the current month
            Dim firstDayOfMonth As New DateTime(DateTime.Now.Year, DateTime.Now.Month, 1)
            Dim lastDayOfMonth As New DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month))

            Using connection As New MySqlConnection(connectionString)
                connection.Open()

                ' Calculate monthly sales
                Dim query As String = $"SELECT SUM(grand_total) FROM orders WHERE transaction_date BETWEEN '{firstDayOfMonth.ToString("yyyy-MM-dd")}' AND '{lastDayOfMonth.ToString("yyyy-MM-dd")}'"

                Using command As New MySqlCommand(query, connection)
                    Dim monthlySales As Object = command.ExecuteScalar()

                    If monthlySales IsNot DBNull.Value Then
                        ' Format sales amount with currency symbol, commas, and "M" for millions
                        lbl_monthlySales.Text = $"₱ {Convert.ToDecimal(monthlySales).ToString("N2", CultureInfo.InvariantCulture)}"
                    Else
                        lbl_monthlySales.Text = "₱ 0.00"
                    End If
                End Using
            End Using
        Catch ex As Exception
            MessageBox.Show($"Error calculating monthly sales: {ex.Message}")
        End Try
    End Sub
    ...
    Private Sub displayAllBtn_Click(sender As Object, e As EventArgs) Handles displayAllBtn.Click
        ...
        LoadAllTransactionData(reportData)
    ...
    Private Sub PrintAllData(dataGridView As DataGridView, e As PrintPageEventArgs)
        ...
        columnWidths(0) = 120 ' Width for the first column (Transaction ID)
        columnWidths(1) = 300 ' Width for the second column (Product Name)
        columnWidths(2) = 150 ' Width for the third column (Sub Total)
        columnWidths(3) = 50  ' Width for the fourth column (Quantity)
        columnWidths(4) = 150 ' Width for the fifth column (Grand Total)
        columnWidths(5) = 120 ' Width for the sixth column (Transaction Date)
        columnWidths(6) = 150 ' Width for the seventh column (Cashier)
        ...
        Dim customHeaders As String() = {"Transaction #", "Product Name", "SubTotal", "Qty", "Total", "Date", "Cashier"}
        ...
        Else
            ...
            ' Calculate total quantities and total amount of grand total
            Dim totalQty As Integer = 0
            Dim totalGrandTotal As Decimal = 0

            For Each row As DataGridViewRow In dataGridView.Rows
                totalQty += CInt(row.Cells(3).Value) ' Assuming the Quantity column is at index 3
                totalGrandTotal += Convert.ToDecimal(row.Cells(4).Value) ' Assuming the Grand Total column is at index 4
            Next
            ' Reset xPos for total rows
            xPos = margin

            ' Print total quantities
            Dim totalQtyText As String = $"Total Orders: {totalQty}"
            e.Graphics.DrawString(totalQtyText, printFont, Brushes.Black, New Rectangle(xPos, yPos, e.MarginBounds.Width, printFont.Height), format)
            yPos += CInt(printFont.GetHeight()) + spaceBetweenSections ' Move to the next line with added space

            ' Print total grand total
            Dim totalGrandTotalText As String = $"Total Grand Total: ₱ {totalGrandTotal}"
            e.Graphics.DrawString(totalGrandTotalText, printFont, Brushes.Black, New Rectangle(xPos, yPos, e.MarginBounds.Width, printFont.Height), format)
            yPos += CInt(printFont.GetHeight()) + spaceBetweenSections ' Move to the next line with added space
